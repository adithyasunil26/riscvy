...
#include "Vinst_memory.h" //Change module name here

using namespace std;

#define MAX_TIME 0.23 //Set max time here in ns 
#define PERIOD 10  //Set simulation period here in ps
static Vinst_memory* ptop = new Vinst_memory;  //Change module name here
...
static void run(uint64_t limit)
{
  ...
  while(count < limit)
  {
    ptop->inst_address = count*4;   //Modify as per module
    tick(PERIOD);                 //
    ++count;
  }
}
